...
public class WebService : System.Web.Services.WebService 
{
    ...
    [WebMethod]
    public static DataSet GetPriceRange()
    {
        da = new SqlDataAdapter("select pricerangeid,pricerange from tbl_pricerange", con);
        ds = new DataSet();
    ...
    [WebMethod]
    public static DataSet GetPriceRangeFor(string ptype, string itemfor, string PriceRange)
    {
        da = new SqlDataAdapter("select tpid.pitemdid,tp.productname,tiop.itemfor,tpr.pricerangeid,tpid.brandname,tpid.imagepath,tpid.cost,tpid.description,tpid.cdate from tbl_productitemdetails tpid inner join tbl_products tp on tpid.pid=tp.pid inner join tbl_itemsofproduct tiop on tp.pid=tiop.pid join tbl_pricerange tpr on tpr.pricerangeid=tpid.pricerangeid where tpid.itemid=tiop.itemid and tp.productname='" + ptype.ToString() + "' and tiop.itemfor='" + itemfor.ToString() + "'and tpr.pricerange='" + PriceRange.ToString() + "' ", con);
        ds = new DataSet();
         da.Fill(ds, "tbl_productitemdetails");
        return ds;
